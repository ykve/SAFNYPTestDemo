...
//    [self addUpperButtonIndex:2];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateSetBadgeValue:)name:kTabBarBadgeValueUpdateNotification object:nil];
    
    [self updateSetBadgeValue:nil];
}

- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kTabBarBadgeValueUpdateNotification object:nil];
}
...
- (void)updateSetBadgeValue:(NSNotification *)notification {
    ...
    BOOL isRefresh = YES;
    if ((self.weChatslastBadgeNum > 100 && [UnreadMessagesNumSingle sharedInstance].weChatsUnReadAllCount > 100) || self.weChatslastBadgeNum == [UnreadMessagesNumSingle sharedInstance].weChatsUnReadAllCount) {
        isRefresh = NO;
    }
    self.weChatslastBadgeNum = [UnreadMessagesNumSingle sharedInstance].weChatsUnReadAllCount;
    
    if (isRefresh) {
        NSString *value = nil;
        if ([UnreadMessagesNumSingle sharedInstance].weChatsUnReadAllCount > 0) {
            if ([UnreadMessagesNumSingle sharedInstance].weChatsUnReadAllCount >= kMessageMaxNum) {
                value = @"99+";
            } else {
                value = [NSString stringWithFormat:@"%ld", [UnreadMessagesNumSingle sharedInstance].weChatsUnReadAllCount];
            }
        }
...
- (void)loadViewControllers {
    ...
    self.zmTabBar.items = @[tabBarItem_1,
                            ...
                            tabBarItem_5];
    //设置默认 显示项
    self.zmTabBar.selectedItem = tabBarItem_3;
