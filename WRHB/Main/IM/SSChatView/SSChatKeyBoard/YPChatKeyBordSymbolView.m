...
//表情视图底部发送和表情筛选部分
@implementation YPChatKeyBordSymbolFooter
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _sendButton .frame = CGRectMake(YPSCREEN_Width-80, 0, 80, self.height);
        _sendButton.tag = 200;
        [_sendButton setTitle:@"发送" forState:UIControlStateNormal];
        _sendButton.backgroundColor = YPChatCellColor;
        ...
        _emojiFooterScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, YPSCREEN_Width - _sendButton.width, self.height)];
        _emojiFooterScrollView.showsHorizontalScrollIndicator = NO;
        _emojiFooterScrollView.showsVerticalScrollIndicator = NO;
        _emojiFooterScrollView.contentSize = CGSizeMake(YPSCREEN_Width - _sendButton.width+1,  self.height);
        [self addSubview:_emojiFooterScrollView];
        
        
        _mButton1 = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, YPSCREEN_Width / 6, 40)];
        _mButton1.tag = 500;
        ...
        [_mButton1 setBackgroundImage:[UIImage imageFromColor:YPChatCellColor] forState:UIControlStateSelected];
...
//        _mButton2 = [[UIButton alloc] initWithFrame:CGRectMake(YPSCREEN_Width / 6, 0, YPSCREEN_Width / 6, 40)];
//        _mButton2.tag = 501;
...
//        [_mButton2 setBackgroundImage:[UIImage imageFromColor:YPChatCellColor] forState:UIControlStateSelected];
...
-(void)buttonPressed:(UIButton *)sender{
    NSLog(@"点击了发送");
    if(_delegate && [_delegate respondsToSelector:@selector(YPChatKeyBordSymbolFooterBtnClick:)]){
        [_delegate YPChatKeyBordSymbolFooterBtnClick:sender];
    }
}
...
//表情视图的表单cell
@implementation YPChatEmojiCollectionCell
...
@implementation YPChatKeyBordSymbolView

-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        self.backgroundColor = YPChatCellColor;
        
        _number = YPChatEmojiRow * YPChatEmojiLine;
        _defaultEmoticons = [NSMutableArray array];
        _emoticonImages = [NSMutableArray array];
        
        
        _footer = [[YPChatKeyBordSymbolFooter alloc] initWithFrame:CGRectMake(0, self.height-YPChatKeyBordSymbolFooterH, YPSCREEN_Width, YPChatKeyBordSymbolFooterH)];
        _footer.backgroundColor = [UIColor whiteColor];
        [self addSubview:_footer];
        _footer.delegate = self;
        
        
        self.layout = [[YPChatCollectionViewFlowLayout alloc] init];

        self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, YPSCREEN_Width, self.height-_footer.height-20) collectionViewLayout:self.layout];
        self.collectionView.backgroundColor = YPChatCellColor;
        self.collectionView.backgroundView.backgroundColor = YPChatCellColor;
        self.collectionView.dataSource = self;
        self.collectionView.delegate = self;
        [self addSubview:self.collectionView];
        [self.collectionView registerClass:[YPChatEmojiCollectionCell class] forCellWithReuseIdentifier:YPChatEmojiCollectionCellId];

        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, _collectionView.bottom, 0, 10)];
        _pageControl.userInteractionEnabled = NO;
        _pageControl.centerX = YPSCREEN_Width*0.5;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    YPChatEmojiCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:YPChatEmojiCollectionCellId forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(YPChatKeyBordSymbolCellClick:)]) {
        [_delegate YPChatKeyBordSymbolCellClick:str];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (scrollView == self.collectionView) {
        if (scrollView.contentOffset.x >= YPSCREEN_Width * _numberPage1){
...
#pragma YPChatKeyBordSymbolFooterDelegate
//底部切换表情500+  发送200
...
//        [self setSymbolValue1];
        ...
        [self.collectionView setContentOffset:CGPointMake(YPSCREEN_Width * _numberPage1, self.collectionView.contentOffset.y) animated:YES];
        [self setSymbolValue2];
        
    }else{
        if(_delegate && [_delegate respondsToSelector:@selector(YPChatKeyBordSymbolViewBtnClick:)]){
            [_delegate YPChatKeyBordSymbolViewBtnClick:sender.tag];
        }
    }
}
...
-(void)setSymbolValue1{
    ...
    self.pageControl.currentPage = (self.collectionView.contentOffset.x / YPSCREEN_Width);
...
-(void)setSymbolValue2{
    ...
    self.pageControl.currentPage = ((self.collectionView.contentOffset.x - _numberPage1*YPSCREEN_Width) / YPSCREEN_Width);
