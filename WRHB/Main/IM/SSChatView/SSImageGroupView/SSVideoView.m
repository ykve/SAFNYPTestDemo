...
-(instancetype)initWithItem:(SSImageGroupItem *)item{
    if(self = [super init]){
        ...
        NSLog(@"视频地址===:  %@  ===",_item.videoPath);
        // 本地视频
        NSURL *videoUrl = [NSURL fileURLWithPath:_item.videoPath];
        if (![_item.videoPath containsString:@".mp4"]) {
            // 在线视频
            videoUrl =  [NSURL URLWithString:[NSString stringWithFormat:@"%@.mp4", _item.videoPath]]; 
        }
        ...
        _playerLayer.frame = CGRectMake(0, 0, YPSCREEN_Width, YPSCREEN_Height);
...
//    __block SSVideoView *weakSelf = self;
    ...
    [_player addPeriodicTimeObserverForInterval:CMTimeMake(1, 30) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        strongSelf.mVideoImagelayer.currentTime = CMTimeGetSeconds(time);
        strongSelf.mVideoImagelayer.totalTime = CMTimeGetSeconds(strongSelf.player.currentItem.duration);
        [strongSelf.mVideoImagelayer setPeriodicTimeAndProgress];
    }];
}
...
-(void)moviePlayDidEnd{
    ...
    [_player seekToTime:CMTimeMake(0, 1) completionHandler:^(BOOL finished) {
        if (finished) {
            ...
            strongSelf.mVideoImagelayer.status = SSVideoLayerValue7;
        } else {
            NSLog(@"478744f1111");
        }
    }];
...
-(void)SSVideoImageLayerButtonClick:(UIButton *)sender item:(SSImageGroupItem *)item{
    ...
    if(sender.tag==100){
        ...
        [_player seekToTime:CMTimeMake(0, 1) completionHandler:^(BOOL finished) {
            ...
            [strongSelf.player pause];
            NSLog(@"=== 234155234 ====");
            strongSelf.mVideoImagelayer.status = SSVideoLayerValue_Closed;
            if(strongSelf.videoViewDelegate && [strongSelf.videoViewDelegate respondsToSelector:@selector(SSVideoViewImageButtonClick:item:)]){
                [strongSelf.videoViewDelegate SSVideoViewImageButtonClick:sender item:item];
            }
         }];
        // 所有才有方法
