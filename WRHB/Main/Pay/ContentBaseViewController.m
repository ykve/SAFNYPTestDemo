...
- (void)viewDidLayoutSubviews {
    ...
    self.listContainerView.frame = CGRectMake(0, Height_NavBar +[self preferredCategoryViewHeight], self.view.bounds.size.width, self.view.bounds.size.height);
}
...
- (id<JXCategoryListContentViewDelegate>)listContainerView:(JXCategoryListContainerView *)listContainerView initListForIndex:(NSInteger)index {
    ...
    if (index== 0 || index == 1) {
        PayTopUpController *list = [[PayTopUpController alloc] init];
        list.isHidTabBar = self.isHidTabBar;
        list.controllerIndex = index;
        return list;
    }
    PayYSTopupController *list = [[PayYSTopupController alloc] init];
     list.isHidTabBar = self.isHidTabBar;
    return list;
