...
@implementation PayWeChatController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"微信存款";
...
- (void)setViewData {
    ...
    if (self.detailsModel.icon.length < kAvatarLength) {
        self.iconImg.image = [UIImage imageNamed:@"pay_wechat_icon"];
    } else {
        [self.iconImg cd_setImageWithURL:[NSURL URLWithString:self.detailsModel.icon] placeholderImage:[UIImage imageNamed:@"pay_wechat_icon"]];
    }
...
- (void)setupUI {
    ...
    topImgView.image = [UIImage imageNamed:@"pay_wechat_topbg"];
...
//    iconImg.backgroundColor = [UIColor redColor];
    iconImg.image = [UIImage imageNamed:@"pay_wechat_icon"];
    ...
    contentLabel.text = @"尊敬的用户您好，您的存款订单已生成，请记录以下官方账户信息以及存款金额，并尽快登录您的微信进行转账，转账完成后请保留银行或APP回执，以遍确认转账信息。";
    ...
    UIImage *image = [UIImage imageNamed:@"pay_wechat_qrbg"];
...
//    [qrImgView setImage:img];
    ...
    tipLabel.text = @"长按保存二维码，打开微信扫一扫选择扫描本地图片";
