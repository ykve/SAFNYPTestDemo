...
@implementation WitAlertHeaderView

///像 自定义cell一样 定义一个headerView
+ (instancetype)VVAlertGroupHeaderViewWithTableView:(UITableView *)tableView {
    static NSString *headerID = @"witheader";
    WitAlertHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:headerID];
    if (headerView == nil) {
...
- (void)setupChlidView{
    ...
    numLabel.backgroundColor = [UIColor colorWithHex:@"#FF6161"];
    
    numLabel.textAlignment = NSTextAlignmentCenter;
    [self.contentView addSubview:numLabel];
    numLabel.layer.cornerRadius = 26/2;
...
- (void)headerBtnClick {
    ...
    if ([self.delegate respondsToSelector:@selector(WitAlertHeaderViewDidClickBtn:)]) {
        
        [self.delegate WitAlertHeaderViewDidClickBtn:self];
    }
}
